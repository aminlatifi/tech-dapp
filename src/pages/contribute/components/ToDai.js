...
const Comp = ({ balance, coin }) => {
    ...
    const aggregatorContract = aggregatorContracts.find((item) => { return item.coin === coin });

    if (!aggregatorContract) {
        return (<div>?</div>);
    }

    const aggregator = new web3.eth.Contract(Aggregator.compilerOutput.abi, aggregatorContract.aggregatorAddress);
    aggregator.methods.latestAnswer().call().then((res) => {
        // debugger;
        const BNbalance = new web3.utils.BN(balance);
        const BNrate = new web3.utils.BN(res);
        const convertedBalance = (BNbalance).div(BNrate); //.div(new web3.utils.BN("10e36"));
        setConvertedBalance(convertedBalance.toNumber(10));
    });
